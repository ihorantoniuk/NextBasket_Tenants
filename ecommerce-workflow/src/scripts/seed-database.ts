...
const sampleProducts: SampleProduct[] = [
  ...
  // Tenant Retail (different tenant)
  {
    id: 'prod-101',
...
export async function seedDatabase(): Promise<void> {
  try {
    ...
    for (const product of sampleProducts) {
      await database.run(insertProduct, [
        product.id,
        product.tenantId,
        product.name,
        product.description,
      ...
      logger.info(`📦 Added product: ${product.name} (${product.tenantId})`);
    }

    logger.info(`✅ Successfully seeded ${sampleProducts.length} products`);
    logger.info('🎯 Tenant demo has 12 products, tenant-retail has 2 products');
