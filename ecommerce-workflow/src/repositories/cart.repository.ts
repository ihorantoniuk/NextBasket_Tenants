...
export class CartRepository {
  ...
  async create(tenantId: string): Promise<Cart> {
    ...
    try {
      ...
      logger.info(`Cart created: ${cart.id} for tenant: ${tenantId}`);
      return cart;
    } catch (error) {
  ...
  async findById(id: string, tenantId: string): Promise<Cart | null> {
    ...
    try {
      const cartRow = await get(
        'SELECT * FROM carts WHERE id = ? AND tenant_id = ?',
        [id, tenantId]
      );
      ...
      logger.error('Failed to find cart by ID:', error);
      throw new Error('Failed to find cart');
    }
  }

  async addItem(cartId: string, tenantId: string, item: AddToCartRequest, productPrice: number): Promise<void> {
    const db = this.getDb();
    const run = promisify(db.run.bind(db));
    const get = promisify(db.get.bind(db));
    ...
    try {
      ...
      const cart = await get(
        'SELECT id FROM carts WHERE id = ? AND tenant_id = ?',
        [cartId, tenantId]
      );
      ...
      logger.info(`Item added to cart: ${cartId}, product: ${item.productId}`);
    } catch (error) {
  ...
  async updateItem(cartId: string, tenantId: string, productId: string, quantity: number): Promise<void> {
    const db = this.getDb();
    const run = promisify(db.run.bind(db));
    const get = promisify(db.get.bind(db));
    ...
    try {
      ...
      logger.info(`Cart item updated: ${cartId}, product: ${productId}`);
    } catch (error) {
  ...
  async removeItem(cartId: string, tenantId: string, productId: string): Promise<void> {
    const db = this.getDb();
    const run = promisify(db.run.bind(db));
    const get = promisify(db.get.bind(db));
    ...
    try {
      ...
      logger.info(`Item removed from cart: ${cartId}, product: ${productId}`);
    } catch (error) {
  ...
  async clear(cartId: string, tenantId: string): Promise<void> {
    const db = this.getDb();
    const run = promisify(db.run.bind(db));
    const get = promisify(db.get.bind(db));
  ...
  async delete(cartId: string, tenantId: string): Promise<boolean> {
    const db = this.getDb();
    const run = promisify(db.run.bind(db));
  ...
  async getPendingStockForProduct(productId: string, tenantId: string): Promise<number> {
    const db = this.getDb();
    const get = promisify(db.get.bind(db));

    try {
      logger.info(`Getting pending stock for product ${productId} in tenant ${tenantId}`);
      ...
      logger.info(`Pending stock for product ${productId}: ${pendingQuantity}`);
      
      return pendingQuantity;
    } catch (error) {
      logger.error('Failed to get pending stock for product:', error);
